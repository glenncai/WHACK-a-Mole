...
void IERG3810_USART2_init(u32 pclk1, u32 bound)
{
	...
	GPIOA->CRL &= 0xffff00ff;	 // set PA2, PA3 Alternate Function (page-138)
	GPIOA->CRL |= 0x00008B00;	 //set PA2, PA3 Alternate Function (page-138)
...
void IERG3810_USART1_init(u32 pclk2, u32 bound)
{
	...
	RCC->APB2ENR |= 1 << 14;	 // USART1EN - Enable USART1 (RM0008 page-142) [different]
	GPIOA->CRH &= 0xfffff00f;	 // set PA9, PA10 Alternate Function [different]
	GPIOA->CRH |= 0x000008B0;	 //set PA9, PA10 Alternate Function [different]
...
void USART_print(u8 USARTport, char *st)
{
	...
	while (st[i] != 0x00)
	{
		if (USARTport == 1)
		{
			USART1->DR = st[i]; // When reading data, reading data to USART_DR will automatically extract RDR data.
			while ((USART1->SR & 0x080) >> 7 != 1)
				; // transmission not finish
		}

		if (USARTport == 2)
		{
			USART2->DR = st[i];
			while ((USART2->SR & 0x080) >> 7 != 1)
				; // transmission not finish
		}
